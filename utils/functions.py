...
def pil_to_tensor(pil):
    return (
        torch.Tensor(np.asarray(pil)).permute(2, 0, 1).unsqueeze(0).float() / 255
    )  # BCHW
...
def depth_to_distance(depth_image, fx=256, fy=256, cx=256, cy=256):
    ...
    grid_x, grid_y = torch.meshgrid(
        torch.arange(depth_image.size(2)), torch.arange(depth_image.size(1))
    )
    ...
    # Calculate distance
    distance_image = depth_image * torch.sqrt(
        ((grid_x - cx) / fx) ** 2 + ((grid_y - cy) / fy) ** 2 + 1
    )
...
def write_video(out_path, images, fps=30, library="imageio"):
    ...
    if library == "imageio":
        ...
        writer = cv2.VideoWriter(
            out_path, fourcc, fps, (images[0].shape[1], images[0].shape[0])
        )
        for image in images:
            writer.write(image)
        writer.release()
...
def write_image(out_path, image):
    ...
    assert (len(image.shape) == 3 and image.shape[-1] in [1, 3]) or len(
        image.shape
    ) == 2
