...
def _verts_to_dirs(pt_a, pt_b, pt_c, gen_res, ratio):
    ...
    to_vec = pt_base + right_vec * 0.5 + down_vec * 0.5
    ...
    pers_ratios = (
        torch.linalg.norm(dirs, 2, -1, True)
        / torch.linalg.norm(to_vec, 2, -1, True)[None, None]
    )

    dirs = dirs / torch.linalg.norm(dirs, 2, -1, True)
    return dirs, pers_ratios, to_vec, down_vec * 0.5, right_vec * 0.5


@torch.no_grad()
def panorama_to_pers_directions(gen_res=512, ratio=1.0, ex_rot=None):
    """
    ...
    ang = np.arctan(0.525731112119133606 / 0.850650808352039932)
    rot_vec = np.array([ang, 0.0, 0.0])
    ...
    for i in range(len(faces)):
        face = faces[i]
        pt_a, pt_b, pt_c = (
            vertices[face[0]].copy(),
            vertices[face[1]].copy(),
            vertices[face[2]].copy(),
        )

        dirs, ratios, to_vec, down_vec, right_vec = _verts_to_dirs(
            pt_a, pt_b, pt_c, gen_res=gen_res, ratio=ratio
        )
...
@torch.no_grad()
def panorama_to_pers_cameras(ratio=1.0):
    """
    Split too may perspective cameras that covers the whole sphere
    :param img: [H, W, C]
    :param gen_res:
    ...
    fovy = torch.arctan(down_vecs_len) * 2.0
    fovx = torch.arctan(right_vecs_len) * 2.0
...
def get_edge_mask(val, threshold=0.01):
    x_laplacian = laplacian(
        val.squeeze()[None, :, :, None].permute(0, 3, 1, 2), kernel_size=3
    )
...
def _get_cliped_mask(x, q):
    q_min = torch.quantile(x, 1.0 - q).item()
