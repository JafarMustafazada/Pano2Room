...
def generate_seed_cubemap():

    thlist = [0, -1 / 2 * np.pi, -1 * np.pi, -3 / 2 * np.pi, 0, 0]
    philist = [0, 0, 0, 0, 1 / 2 * np.pi, -1 / 2 * np.pi]
...
def get_pcdGenPoses(pcdgenpath, argdict={}):
    if pcdgenpath == "rotate360":
        render_poses = generate_seed_360(360, 6)
    ...
    elif pcdgenpath == "cubemap":
        render_poses = generate_seed_cubemap()
    else:
...
def get_camerapaths():
    preset_json = {}
    for cam_path in ["back_and_forth", "llff", "headbanging", "rotate360", "cubemap"]:
        ...
        elif cam_path == "rotate360":
            render_poses = generate_seed_360(360, 360)
        elif cam_path == "cubemap":
            render_poses = generate_seed_cubemap()
        else:
            raise ("Unknown pass")
