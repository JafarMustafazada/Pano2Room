...
class BoundedRays:
    ...
    def __getitem__(self, indices):
        return BoundedRays(
            self.o[indices], self.d[indices], self.near[indices], self.far[indices]
        )
...
def cam_rays_cam_space(height: int, width=-1, fovy=np.deg2rad(90.0), aspect_ratio=1.0):
    """
    OpenCV style!
    :param height:
    :param width:
...
def ang2vec(angles):
    ...
    vecs = torch.stack(
        [
            torch.cos(ang_x) * torch.cos(ang_y),
...
def img_coord_from_hw(h, w):
    i = torch.linspace(0.5 / h, 1.0 - 0.5 / h, h)
    j = torch.linspace(0.5 / w, 1.0 - 0.5 / w, w)
...
def img_to_pano_coord(coords):
    ...
    return torch.stack([-(y - 0.5) * np.pi, -(x - 0.5) * 2.0 * np.pi], -1)


def pano_to_img_coord(coords):
    y, x = coords[..., 0], coords[..., 1]
    return torch.stack([-y / np.pi + 0.5, -(x / (2.0 * np.pi)) + 0.5], -1)
...
def direction_to_pers_img_coord(dirs, to_vec, down_vec, right_vec):
    ...
    mask = (mask & (i.abs() <= 1.0) & (j.abs() <= 1.0)).float()
    ij = (torch.cat([i, j], dim=-1) + 1.0) * 0.5
    return ij, mask


def img_coord_to_sample_coord(coords):
    return torch.stack([coords[..., 1], coords[..., 0]], -1) * 2.0 - 1.0


def get_rand_horizontal_points(batch_size, dim=3):
    rs = torch.sqrt(torch.rand(batch_size))
    theta = (torch.rand(batch_size) * 2.0 - 1.0) * np.pi
...
def pers_depth_to_normal(depth, down_len, right_len):
    ...
    ii, jj = torch.meshgrid(
        torch.linspace(0.5 / h, 1.0 - 0.5 / h, h),
        torch.linspace(0.5 / w, 1.0 - 0.5 / w, w),
        indexing="ij",
    )
    z = torch.ones_like(ii)
    x = (jj * 2.0 - 1.0) * right_len
    y = (ii * 2.0 - 1.0) * down_len
