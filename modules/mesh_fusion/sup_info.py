...
class PanoSupInfo(SupInfo):
    def __init__(self, pose, mask, color_map, distance_map, normal_map=None, factor=1):
        ...
        if factor != 1:
            ...
            color_map = cv.resize(
                color_map.cpu().numpy(), (width, height), interpolation=cv.INTER_AREA
            )
            distance_map = cv.resize(
                distance_map.cpu().numpy(), (width, height), interpolation=cv.INTER_AREA
            )
            normal_map = cv.resize(
                normal_map.cpu().numpy(), (width, height), interpolation=cv.INTER_AREA
            )
        ...
        x_laplacian = kornia.filters.laplacian(
            distance_map[None].permute(0, 3, 1, 2), kernel_size=3
        )
        ...
        mask = mask & (edge_mask[0] > 0.5).permute(1, 2, 0)

        if has_normal_map:
            pano_dirs = -img_coord_to_pano_direction(img_coord_from_hw(height, width))
            normal_cos = (pano_dirs * normal_map).sum(-1, True).clip(0.0, 1.0)
    ...
    def update_sup_info(self):
        ...
        img_coords = torch.meshgrid(
            torch.linspace(0.5 / height, 1.0 - 0.5 / height, height),
        ...
        sup_colors, sup_distances, sup_normals = (
            self.color_map[sup_indices],
            self.distance_map[sup_indices],
            self.normal_map[sup_indices],
        )
    ...
    def get_pers_patch_data(self, res, fov, from_masked_region=True):
        ...
        if not from_masked_region:
            ...
            coords = torch.stack(
                [
                    (self.sup_indices[0] + 0.5) / self.height,
                    (self.sup_indices[1] + 0.5) / self.width,
                ],
                dim=-1,
            )
        ...
        colors = F.grid_sample(
            self.color_map[None].permute(0, 3, 1, 2), sampled_coords[None]
        )[0].permute(1, 2, 0)
        rays = Rays(
            torch.zeros_like(local_pers_rays_d) + self.pose[:3, 3],
            apply_rot(local_pers_rays_d, self.pose[:3, :3]),
        )
...
class SupInfoPool:
    ...
    def register_sup_info(self, pose, mask, rgb, distance, normal=None):
        ...
        if self.all_sup_colors is None:
            ...
            self.all_sup_colors = torch.cat(
                [self.all_sup_colors, self.sup_infos[-1].sup_colors], 0
            )
            self.all_sup_rays = cat_rays(
                [self.all_sup_rays, self.sup_infos[-1].sup_rays]
            )
            self.all_sup_distances = torch.cat(
                [self.all_sup_distances, self.sup_infos[-1].sup_distances], 0
            )
            self.all_sup_normals = torch.cat(
                [self.all_sup_normals, self.sup_infos[-1].sup_normals], 0
            )
    ...
    def geo_check(self, rays, distances):
        ...
        for pano_idx in range(len(self.sup_infos)):
            ...
            proj_distances = F.grid_sample(
                sup_distance_map[None].permute(0, 3, 1, 2),
                sample_coords[None],
                padding_mode="border",
            )
    ...
    def gen_occ_grid(self, res):
        ...
        for shift_xyz in shift_xyzs:
            ...
            shifted = ((shifted.clip(-0.999, 0.999) * 0.5 + 0.5) * res).to(torch.int64)
            shifted_idx = (
                shifted[..., 0] * res * res + shifted[..., 1] * res + shifted[..., 2]
            )
            assert (
                shifted_idx.max().item() < res * res * res
                and shifted_idx.min().item() >= 0
            )
        ...
        valid_pts = (valid_pts / float(res) - 0.5) * 2.0
    ...
    def load_state_dict(self, state_dict):
        ...
        self.all_sup_distances = torch.cat(
            [info.sup_distances for info in self.sup_infos], 0
        )
        self.all_sup_normals = torch.cat(
            [info.sup_normals for info in self.sup_infos], 0
        )
