...
class PanoPersFusionInpainter(Inpainter):
    ...
    def inpaint(self, idx, img, mask):
        ...
        pers_dirs, pers_ratios, to_vecs, down_vecs, right_vecs = (
            panorama_to_pers_directions(gen_res=512, ratio=1.4)
        )
        ...
        for i in tqdm(range(n_pers)):
            ...
            pers_image = F.grid_sample(
                inpainted_img[None], cur_sample_coords[None], padding_mode="border"
            )[0]
            pers_mask = F.grid_sample(
                mask[None, :, :], cur_sample_coords[None], padding_mode="border"
            )[0]
            ...
            if self.lama_inpainter is not None:
                ...
                lama_inpainted = self.lama_inpainter.inpaint(
                    pers_image[None], pers_mask[None]
                )[0]
                if smooth_mask.max().item() > 0.5:
                    cur_inpainted = self.diff_inpainter.inpaint(
                        lama_inpainted[None], smooth_mask[None]
                    )[0]
                else:
                    cur_inpainted = lama_inpainted
            else:
                if pers_mask.max().item() > 0.5:
                    cur_inpainted = self.diff_inpainter.inpaint(
                        pers_image[None], pers_mask[None]
                    )[0]
                else:
            ...
            cur_inpainted_pano_img = F.grid_sample(
                cur_inpainted[None], proj_coord[None], padding_mode="border"
            )[0]
            proj_mask = proj_mask.permute(2, 0, 1).float()
            inpainted_img = (
                inpainted_img * (1.0 - proj_mask) + cur_inpainted_pano_img * proj_mask
            )
            mask = mask * (1.0 - proj_mask) + 0.0 * proj_mask
