...
from modules.inpainters.lama.saicinpainting.training.data.datasets import (
    make_constant_area_crop_params,
)
from modules.inpainters.lama.saicinpainting.training.losses.distance_weighting import (
    make_mask_distance_weighter,
)
from modules.inpainters.lama.saicinpainting.training.losses.feature_matching import (
    feature_matching_loss,
    masked_l1_loss,
)
from modules.inpainters.lama.saicinpainting.training.modules.fake_fakes import (
    FakeFakesGenerator,
)
from modules.inpainters.lama.saicinpainting.training.trainers.base import (
    BaseInpaintingTrainingModule,
    make_multiscale_noise,
)
from modules.inpainters.lama.saicinpainting.utils import add_prefix_to_keys, get_ramp
...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def generator_loss(self, batch):
        ...
        if self.config.losses.perceptual.weight > 0:
            pl_value = (
                self.loss_pl(predicted_img, img, mask=supervised_mask).sum()
        ...
        mask_for_discr = (
            supervised_mask if self.distance_weighted_mask_for_discr else original_mask
        )
        ...
        if self.config.losses.feature_matching.weight > 0:
            need_mask_in_fm = OmegaConf.to_container(
                self.config.losses.feature_matching
            ).get("pass_mask", False)
            mask_for_fm = supervised_mask if need_mask_in_fm else None
            fm_value = (
                feature_matching_loss(
                    discr_fake_features, discr_real_features, mask=mask_for_fm
                )
    ...
    def discriminator_loss(self, batch):
        ...
        if batch.get("use_fake_fakes", False):
            ...
            fake_fakes_adv_discr_loss, fake_fakes_adv_metrics = (
