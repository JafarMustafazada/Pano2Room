...
class InpaintingEvaluator:
    ...
    def _get_bin_edges(self):
        ...
        for idx_bin in range(self.bins):
            start_percent, end_percent = round(
                100 * bin_edges[idx_bin], num_digits
        ...
        for batch in self.dataloader:
            ...
            bin_indices = (
                np.searchsorted(bin_edges, area.detach().cpu().numpy(), side="right")
                - 1
            )
        ...
        for score_name, score in tqdm.auto.tqdm(self.scores.items(), desc="scores"):
            ...
            with torch.no_grad():
                ...
                for batch in tqdm.auto.tqdm(
                    self.dataloader, desc=score_name, leave=False
                ):
...
class InpaintingEvaluatorOnline(nn.Module):
    def __init__(
        ...
        for idx_bin in range(self.bins_num):
            start_percent, end_percent = round(
                100 * self.bin_edges[idx_bin], num_digits
    ...
    def _get_bins(self, mask_batch):
        ...
        bin_indices = np.clip(
            np.searchsorted(self.bin_edges, area) - 1, 0, self.bins_num - 1
        )
        return bin_indices
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        result = {}
        with torch.no_grad():
            image_batch, mask_batch, inpainted_batch = (
                batch[self.image_key],
                batch["mask"],
                batch[self.inpainted_key],
            )
            if self.clamp_image_range is not None:
                image_batch = torch.clamp(
                    image_batch,
