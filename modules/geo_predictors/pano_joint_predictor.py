...
class SphereDistanceField(nn.Module):
    def __init__(self, n_levels=16, log2_hashmap_size=19, base_res=16, fine_res=2048):
        super().__init__()
    ...
    def forward(self, directions, requires_grad=False):
        ...
        selector = (
            ((dir_scaled > 0.0) & (dir_scaled < 1.0)).all(dim=-1).to(torch.float32)
        )
        scene_feat = self.hash_grid(dir_scaled)

        distance = F.softplus(
            self.geo_mlp(torch.cat([directions, scene_feat], -1))[..., 0] + 1.0
        )
...
class PanoJointPredictor(GeoPredictor):
    ...
    def grads_to_normal(self, grads):
        ...
        is_inside = ((normals * pano_dirs).sum(-1, True) < 0.0).float()
        normals = normals * is_inside + -normals * (1.0 - is_inside)
        return normals
    ...
    def __call__(
        ...
        ref_distance = (
            ref_distance.clone().squeeze()[..., None].float().permute(2, 0, 1)
        )
        ...
        for ratio in [1.4]:
            (
                ...
                cur_down_vecs,
                cur_right_vecs,
            ) = panorama_to_pers_directions(gen_res=gen_res, ratio=ratio)
        ...
        fx = (
            torch.linalg.norm(to_vecs, 2, -1, True)
            / torch.linalg.norm(right_vecs, 2, -1, True)
            * gen_res
            * 0.5
        )
        fy = (
            torch.linalg.norm(to_vecs, 2, -1, True)
            / torch.linalg.norm(down_vecs, 2, -1, True)
            * gen_res
            * 0.5
        )
        cx = torch.ones_like(fx) * gen_res * 0.5
        cy = torch.ones_like(fy) * gen_res * 0.5
        ...
        rot_w2c = torch.stack(
            [
                right_vecs / torch.linalg.norm(right_vecs, 2, -1, True),
        ...
        pers_imgs = F.grid_sample(
            img[None].expand(n_pers, -1, -1, -1), sample_coords, padding_mode="border"
        )
        pred_distances_raw = []
        pred_normals_raw = []

        pers_distances = F.grid_sample(
            ref_distance[None].expand(n_pers, -1, -1, -1),
            sample_coords,
            padding_mode="border",
        )
        pers_masks = F.grid_sample(
            mask[None].expand(n_pers, -1, -1, -1), sample_coords, padding_mode="border"
        )
        ...
        for i in range(n_pers):
            with torch.no_grad():
                ...
                pred_depth = (
                    self.depth_predictor.predict_depth(
                        pers_imgs[i : i + 1],
                        intri=intri,
                        pers_distance=pers_distances[i : i + 1],
                        pers_mask=pers_masks[i : i + 1],
                    )
                    .cuda()
                    .clip(0.0, None)
                )  # [1, 1, res, res]
                pred_depth = pred_depth / (pred_depth.mean() + 1e-5)
                pred_distances_raw.append(
                    pred_depth * pers_ratios[i].permute(2, 0, 1)[None]
                )

                pred_normals = self.normal_predictor.predict_normal(
                    pers_imgs[i : i + 1]
                )
                pred_normals = pred_normals * 2.0 - 1.0
                pred_normals = pred_normals / torch.linalg.norm(
                    pred_normals, ord=2, dim=1, keepdim=True
                )
        ...
        bias_params_local_distance = torch.zeros(
            [n_pers, 1, gen_res, gen_res], requires_grad=True
        )
        bias_params_local_normal = torch.zeros(
        ...
        optimizer_global = torch.optim.Adam(
            [scale_params, bias_params_global], lr=init_lr
        )
        optimizer_local = torch.optim.Adam(
            [bias_params_local_distance, bias_params_local_normal], lr=init_lr_local
        )
        ...
        for phase in ["global", "hybrid"]:
            for iter_step in tqdm(range(all_iter_steps)):
                ...
                lr_ratio = (np.cos(progress * np.pi) + 1.0) * (
                    1.0 - lr_alpha
                ) + lr_alpha
                ...
                sample_coords = torch.rand(n_pers, local_batch_size, 1, 2) * 2.0 - 1
                cur_sup_info = F.grid_sample(
                    sup_infos, sample_coords, padding_mode="border"
                )
                distance_bias = F.grid_sample(
                    bias_params_local_distance, sample_coords, padding_mode="border"
                )
                distance_bias = distance_bias[:, :, :, 0].permute(0, 2, 1)
                normal_bias = F.grid_sample(
                    bias_params_local_normal, sample_coords, padding_mode="border"
                )
                ...
                ref_pred_distances = ref_pred_distances * F.softplus(
                ...
                pred_distances, pred_grads = sp_dis_field(
                    dirs.reshape(-1, 3), requires_grad=True
                )
                ...
                distance_loss = F.smooth_l1_loss(
                    ref_pred_distances, pred_distances, beta=5e-1, reduction="mean"
                )
                ...
                normal_loss = F.smooth_l1_loss(
                    errors, torch.zeros_like(errors), beta=5e-1, reduction="mean"
                )

                reg_loss = ((F.softplus(scale_params).mean() - 1.0) ** 2).mean()

                if phase == "hybrid":
                    distance_bias_local = bias_params_local_distance
                    distance_bias_tv_loss = F.smooth_l1_loss(
                        distance_bias_local[:, :, 1:, :],
                        distance_bias_local[:, :, :-1, :],
                        beta=1e-2,
                    ) + F.smooth_l1_loss(
                        distance_bias_local[:, :, :, 1:],
                    ...
                    normal_bias_tv_loss = F.smooth_l1_loss(
                        normal_bias_local[:, :, 1:, :],
                        normal_bias_local[:, :, :-1, :],
                        beta=1e-2,
                    ) + F.smooth_l1_loss(
                ...
                ref_distance_loss = F.smooth_l1_loss(
                    sampled_ref_distance.reshape(-1),
                    pred_distances.reshape(-1),
                    beta=1e-2,
                    reduction="none",
                )
                ref_distance_loss = (
                    ref_distance_loss * (sampled_ref_mask < 0.5).reshape(-1)
                ).mean()

                loss = (
                    ref_distance_loss * 20.0 * progress
                    + distance_loss
                    + reg_loss * reg_loss_weight
                    + normal_loss * normal_loss_weight
                    + distance_bias_tv_loss * 1.0
        ...
        pano_dirs = img_coord_to_pano_direction(img_coord_from_hw(height, width))
        new_distances, new_grads = sp_dis_field(
            pano_dirs.reshape(-1, 3), requires_grad=True
        )
