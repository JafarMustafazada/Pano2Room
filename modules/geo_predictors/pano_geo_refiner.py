...
class SphereDistanceField(nn.Module):
    def __init__(self, n_levels=16, log2_hashmap_size=19, base_res=16, fine_res=4096):
        super().__init__()
...
class PanoGeoRefiner:
    ...
    def refine(self, distances, normals):
        ...
        for iter_i in tqdm(range(n_iters)):
            ...
            ref_distance = F.grid_sample(
                distances[None], sample_coord[None, None], padding_mode="border"
            )[0, 0, 0]
            ref_normal = (
                F.grid_sample(
                    normals[None], sample_coord[None, None], padding_mode="border"
                )[0, :, 0]
                .permute(1, 0)
                .contiguous()
            )
            ...
            distance_loss = F.smooth_l1_loss(
                ref_distance, pred_distance, beta=1e-2, reduction="mean"
            )
            ...
            lr_factor = (np.cos(progress * np.pi) * 0.5 + 0.5) * (
                1.0 - lr_alpha
            ) + lr_alpha
