...
class PanoFusionNormalPredictor(GeoPredictor):
    ...
    def inpaint_normal(self, img, ref_normal, mask, gen_res=384):
        ...
        pers_dirs, pers_ratios, to_vecs, down_vecs, right_vecs = (
            panorama_to_pers_directions(gen_res=gen_res, ratio=1.0)
        )
        ...
        pano_img_coords = torch.meshgrid(
            torch.linspace(0.5 / pano_height, 1.0 - 0.5 / pano_height, pano_height),
            torch.linspace(0.5 / pano_width, 1.0 - 0.5 / pano_width, pano_width),
            indexing="ij",
        )
        ...
        for i in range(n_pers):
            with torch.no_grad():
                ...
                pred_normal = self.normal_predictor.predict_normal(
                    pers_imgs[i : i + 1]
                ).cuda()
                pred_normal = pred_normal * 2.0 - 1.0
                pred_normal = pred_normal / torch.linalg.norm(
                    pred_normal, ord=2, dim=1, keepdim=True
                )
        ...
        for i in range(n_pers):
            proj_coord, proj_mask = direction_to_pers_img_coord(
                pano_dirs, to_vecs[i], down_vecs[i], right_vecs[i]
            )
        ...
        proj_coords = torch.cat(
            proj_coords, dim=0
        )  # [n_pers, pano_height, pano_width, 2]
        proj_masks = torch.cat(
            proj_masks, dim=0
        )  # [n_pers, pano_height, pano_width, 1]
        proj_coords = proj_coords.clip(-1.0, 1.0)  # not necessary
        ...
        pano_normal_params = -img_coord_to_pano_direction(
            img_coord_from_hw(pano_height, pano_width)
        )
        ...
        for iter_step in tqdm(range(all_iter_steps)):
            ...
            lr_ratio = (np.cos(lr_progress * np.pi) + 1.0) * (1.0 - lr_alpha) + lr_alpha
            for g in optimizer.param_groups:
                g["lr"] = init_lr * lr_ratio

            ii, jj = torch.meshgrid(
                torch.linspace(-1.0, 1.0, gen_res), torch.linspace(-1.0, 1.0, gen_res)
            )

            pano_normal = pano_normal_params * mask + ref_normal * (1.0 - mask)
            pano_normal_norm = torch.linalg.norm(
                pano_normal, ord=2, dim=0, keepdim=True
            )
            pano_normal = pano_normal / pano_normal_norm

            pred_normals = pred_normals_raw + bias_params
            pred_normals_norm = torch.linalg.norm(
                pred_normal, ord=2, dim=1, keepdim=True
            )
            pred_normals = pred_normals / pred_normals_norm
            pred_normals = F.grid_sample(
                pred_normals, proj_coords, padding_mode="border"
            )

            align_error = (pred_normals - pano_normal[None]) * proj_masks
            align_error = F.smooth_l1_loss(
                align_error, torch.zeros_like(align_error), beta=0.5, reduction="none"
            )
            weight_masks = proj_masks * distortion_weights[None, None]
            align_loss = (align_error * weight_masks).sum() / weight_masks.sum()

            bias_tv_loss = F.smooth_l1_loss(
                bias_params[:, :, 1:, :], bias_params[:, :, :-1, :], beta=0.5
            ) + F.smooth_l1_loss(
                bias_params[:, :, :, 1:], bias_params[:, :, :, :-1], beta=0.5
            )

            reg_loss = ((pano_normal_norm - 1.0) ** 2).mean() + (
                (pred_normals_norm - 1.0) ** 2
            ).mean()
            loss = align_loss + bias_tv_loss * 1.0 + reg_loss * 1e-2
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
