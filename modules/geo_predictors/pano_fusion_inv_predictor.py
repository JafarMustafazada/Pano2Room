...
from utils.camera_utils import (
    img_coord_to_sample_coord,
    img_to_pano_coord,
    direction_to_img_coord,
    img_coord_to_pano_direction,
    direction_to_pers_img_coord,
    pers_depth_to_normal,
)
...
class PanoFusionInvPredictor(GeoPredictor):
    ...
    def inpaint_distance(self, img, ref_distance, mask, gen_res=384):
        ...
        pano_distance_params = torch.zeros(
            [1, pano_height, pano_width], requires_grad=True
        )
        ...
        optimizer_b = torch.optim.Adam(
            [scale_params, bias_params, pano_distance_params], lr=init_lr
        )
        ...
        for iter_step in tqdm(range(all_iter_steps)):
            ...
            lr_ratio = (np.cos(progress * np.pi) + 1.0) * (1.0 - lr_alpha) + lr_alpha
            ...
            pano_distance = pano_distance * mask + ref_distance * (1.0 - mask)
            ...
            if phase == "scale_only":
                ...
                bias = bias_params
            pred_distances = (
                (pred_depths_raw + bias)
                * pers_ratios.permute(0, 3, 1, 2)
                * scales[:, None, None, None]
            )
            pred_distances = pred_distances.clip(1e-5, None)
            pred_distances = F.grid_sample(
                pred_distances, proj_coords, padding_mode="border"
            )
            ...
            align_error = F.smooth_l1_loss(
                align_error, torch.zeros_like(align_error), beta=1e-1, reduction="none"
            )
            ...
            bias_tv_loss = F.smooth_l1_loss(
                bias_params[:, :, 1:, :], bias_params[:, :, :-1, :], beta=1e-1
            ) + F.smooth_l1_loss(
                bias_params[:, :, :, 1:], bias_params[:, :, :, :-1], beta=1e-1
            )

            reg_loss = (scales.mean() - 1.0) ** 2
