...
MAKE_RESCALE_0_1_NEG1_POS1 = lambda n_chan: transforms.Normalize(
    [0.5] * n_chan, [0.5] * n_chan
)
RESCALE_0_1_NEG1_POS1 = transforms.Normalize(
    [0.5], [0.5]
)  # This needs to be different depending on num out chans
MAKE_RESCALE_0_MAX_NEG1_POS1 = lambda maxx: transforms.Normalize(
    [maxx / 2.0], [maxx * 1.0]
)
...
def get_transform(task: str, image_size=Optional[int]):
    ...
    if task in ["rgb", "normal", "reshading"]:
        ...
        "segment_instance",
        "fragments",
    ]:  # this is stored as 1 channel image (H,W) where each pixel value is a different class
    ...
    if "clamp_to" in task_configs.task_parameters[task]:
        ...
        if minn > 0:
            raise NotImplementedError(
                "Rescaling (min1, max1) -> (min2, max2) not implemented for min1, min2 != 0 (task {})".format(
                    task
                )
            )
        transform = transforms.Compose([transform, MAKE_RESCALE_0_MAX_0_POS1(maxx)])

    if image_size is not None:
        if task == "fragments":
            resize_frag = (
                lambda frag: torch.nn.functional.interpolate(
                    frag.permute(2, 0, 1).unsqueeze(0).float(),
                    image_size,
                    mode="nearest",
                )
                .long()[0]
            ...
            transform = transforms.Compose([transform, resize_frag])
        else:
            ...
            transform = transforms.Compose(
                [transforms.Resize(image_size, resize_method), transform]
            )
...
transform_dense_labels = lambda img: torch.Tensor(
    np.array(img)
).long()  # avoids normalizing
...
class LocalContrastNormalization(object):
    ...
    def __call__(self, tensor):
        ...
        arr = np.array(tensor)
        local_sum_arr = np.array(
            [convolve(arr[c], kernel, mode=self.mode, cval=self.cval) for c in range(C)]
        ...
        local_avg_arr = local_sum_arr / (
            self.kernel_size**2
        )  # The tensor of local averages.

        arr_square = np.square(arr)
        local_sum_arr_square = np.array(
            [
                convolve(arr_square[c], kernel, mode=self.mode, cval=self.cval)
                for c in range(C)
            ]
        )  # An array that has shape(C, H, W)
        # Each element [c, h, w] is the summation of the values
        # in the window that has arr_square[c,h,w] at the center.
        local_norm_arr = np.sqrt(
            local_sum_arr_square
        )  # The tensor of local Euclidean norms.
    ...
    def __repr__(self):
        return self.__class__.__name__ + "(kernel_size={0}, threshold={1})".format(
            self.kernel_size, self.threshold
        )
